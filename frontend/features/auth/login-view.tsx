...
export function LoginView() {
  ...
  const [remember, setRemember] = useState(
    () =>
      ...
      window.localStorage.getItem("remember_account") === "true",
  );
  ...
  const [form, setForm] = useState(() => ({
    username:
      typeof window === "undefined"
        ? ""
        : window.localStorage.getItem("remembered_username") || "",
    password: "",
    registerToken: "",
  }));
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const result =
        mode === "login"
          ...
          : await register(
              ...
              form.registerToken || undefined,
            );
      if (!result.ok) {
        setError(
          result.message ||
            (mode === "login" ? "用户名或密码不正确" : "注册失败，请检查输入"),
        );
        return;
      }
      ...
      setError(
        caught instanceof ApiNetworkError
          ? caught.message
          : "登录服务暂时不可用",
      );
    } finally {
  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-[minmax(0,1.1fr)_minmax(420px,.9fr)]">
      <section className="relative hidden overflow-hidden border-r border-border bg-secondary p-12 text-foreground lg:flex lg:flex-col lg:justify-between xl:p-16">
        ...
        <div className="relative flex items-center gap-3">
          <span className="grid size-11 place-items-center rounded-xl bg-background text-foreground">
        ...
        <div className="relative max-w-2xl">
          ...
          <h1 className="mt-5 text-pretty font-display text-6xl font-semibold leading-[.95] tracking-[-0.045em] xl:text-7xl">
            把每一集，留在自己的时间线上。
          </h1>
          <p className="mt-6 max-w-xl text-lg leading-relaxed text-muted-foreground">
            从搜索、进度到单集记录，所有变化都清楚可查。打开片库，继续上次停下的位置。
          </p>
          <div className="mt-10 flex items-center gap-1" aria-hidden>
            {Array.from({ length: 18 }, (_, index) => (
              <span
                key={index}
                className={
                  index < 11
                    ? "h-2 flex-1 bg-primary"
                    : "h-2 flex-1 bg-primary/10"
                }
              />
            ))}
          </div>
        ...
        <div className="relative font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
          Private library / Same-origin API / Dark ready
        </div>
      </section>
            ...
            <form onSubmit={submit} noValidate className="grid gap-5">
              ...
              <div className="grid gap-2">
                ...
                <div className="relative">
                  ...
                  <button
                    type="button"
                    className="absolute right-1 top-1/2 grid size-9 -translate-y-1/2 place-items-center rounded-md text-muted-foreground hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              ...
              {mode === "register" && registerNeedToken ? (
                <div className="grid gap-2">
                  <Label htmlFor="register-token">注册令牌</Label>
                  <Input
                    id="register-token"
                    type="password"
                    autoComplete="off"
                    value={form.registerToken}
                    onChange={(event) =>
                      setForm({ ...form, registerToken: event.target.value })
                    }
                  />
                </div>
              ) : null}
              ...
              <Button type="submit" size="lg" loading={loading}>
                {mode === "login" ? "登录并打开片库" : "创建账户"}
              </Button>
            </form>
          </Card>
          {allowRegister ? (
            <button
              className="mt-6 min-h-11 w-full text-sm font-semibold text-muted-foreground underline-offset-4 hover:text-foreground hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              onClick={() => {
